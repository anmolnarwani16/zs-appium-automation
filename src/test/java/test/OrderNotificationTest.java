...
public class OrderNotificationTest extends MobileBaseTest {
    /**
    ...
    public void orderNotification(Map<String, String> data) {
        new HomePage().clickOnFruit(Constant.ORDER_NOTIFICATION);
        for (int i = 0; i < 3; i++) {
            new FruitCategoryPage().clickOnAddIcon(Constant.ORDER_NOTIFICATION);
        }
        ...
        String value = new OrderNotificationPage().getOrderId(Constant.ORDER_NOTIFICATION, "orderText");
        System.out.println(value);
        new OrderNotificationPage().clickOnBack(Constant.ORDER_NOTIFICATION);
        new HomePage().clickOnNotificationIcon(Constant.ORDER_NOTIFICATION);
        String value1 = new OrderNotificationPage().getOrderId(Constant.ORDER_NOTIFICATION, "orderNumber");
